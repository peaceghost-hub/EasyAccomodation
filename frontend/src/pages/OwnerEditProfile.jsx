...
export default function OwnerEditProfile() {
  ...
  const [form, setForm] = useState({ full_name: '', email: '', phone_number: '', ecocash_number: '', bank_account: '', other_payment_info: '', current_password: '', new_password: '', confirm_new_password: '' });
  ...
  useEffect(() => {
    if (user) {
      setForm({
        ...
        ecocash_number: user.owner_info?.ecocash_number || user.owner_info?.ecocash || '',
        bank_account: user.owner_info?.bank_account || '',
        other_payment_info: user.owner_info?.other_payment_info || ''
      });
    }
  }, [user]);
    ...
    try {
      const payload = {
        ...
        owner_profile: {
          ...
          other_payment_info: form.other_payment_info
        }
      };
      ...
      setTimeout(() => navigate('/owner/house'), 700);
    } catch (err) {
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input className="input w-full" value={form.ecocash_number} onChange={handleChange('ecocash_number')} />
          </div>
          <div>
            <label className="text-sm font-medium">Bank Account</label>
            <input className="input w-full" value={form.bank_account} onChange={handleChange('bank_account')} />
          </div>
          <div>
            <label className="text-sm font-medium">Other payment info</label>
            <textarea className="input w-full" value={form.other_payment_info} onChange={handleChange('other_payment_info')} />
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => navigate('/owner/house')} className="px-4 py-2 bg-gray-100 rounded">Cancel</button>
          </div>
        </div>
      </div>
    </div>
