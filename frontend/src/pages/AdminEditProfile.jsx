...
export default function AdminEditProfile() {
  ...
  const [form, setForm] = useState({ full_name: '', email: '', phone_number: '', current_password: '', new_password: '', confirm_new_password: '' });
  ...
  const handleSave = async () => {
    ...
    setMessage('');
    try {
      const payload = { full_name: form.full_name, email: form.email, phone_number: form.phone_number };
      const res = await adminAPI.updateUser(user.id, payload);
      // update local context with returned user if available, else with our payload
      const updated = res.data?.user || { ...user, ...payload };
      updateLocalUser(updated);
      // If user requested password change, validate and call changePassword
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => navigate('/admin')} className="px-4 py-2 bg-gray-100 rounded">Cancel</button>
          </div>
        </div>
      </div>
    </div>
